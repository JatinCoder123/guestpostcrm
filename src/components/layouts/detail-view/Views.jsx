...
function SortableNode({ item, scope, layout, selected, disabled, onSelect, onToggle, onAdd }) {
  ...
  return <div ref={sortable.setNodeRef} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }} className={`${isContainer ? "overflow-hidden rounded-xl border bg-card" : "rounded-lg"} ${active ? "border-primary/40 bg-primary/[0.03] shadow-sm" : isContainer ? "border-border" : ""} ${sortable.isDragging ? "opacity-40" : ""}`}>
    <div onClick={() => onSelect(scope, item)} className={`layout-tree-row flex cursor-pointer items-center gap-2 px-2 ${isContainer ? "py-2.5" : "py-2 hover:bg-accent/60"}`}>
      <button type="button" {...sortable.attributes} {...sortable.listeners} onClick={(e) => e.stopPropagation()} disabled={disabled} className="grid h-7 w-5 shrink-0 touch-none cursor-grab place-items-center rounded text-muted-foreground/40 hover:bg-accent hover:text-foreground active:cursor-grabbing disabled:opacity-30"><GripVertical className="h-3.5 w-3.5" /></button>
      ...
      <span className="hidden sm:inline max-w-16 truncate font-mono text-[9px] text-muted-foreground">{item.rank || "—"}</span>
