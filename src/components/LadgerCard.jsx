...
function ChildCard({ parentId, handleMessageClick }) {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isPending } =
    useInfiniteChildLedger(parentId);
  ...
  return (
    <div className="mt-5 ml-2 space-y-4 relative">
      ...
      {ladgerChild?.map((child, index) => {
        ...
        return (
          <div key={child.id} ref={isLast ? lastChildRef : null}>
            <ChildItem
              child={child}
              Icon={child.icon}
    ...
    >
      ...
      <div
        className={`
                    ...
                    ${isHovered ? "bg-primary/20" : "bg-card"}
                    z-20
                `}
      ></div>
          ...
          >
            ...
            <div className="min-h-0 min-w-0 flex-1 overflow-hidden p-2 sm:p-4">
              ...
              {activeTab === "prompt" && (
                <PromptLadger
                  isModal={false}
                  activePromptId={child.prompt_ledger_id}
                  setActivePromptId={setActivePromptId}
                />
              )}
              ...
              {activeTab === "visualization" && (
                <Visualization
                  isModal={false}
                  activeVisualizationId={child.stage_ledger_id}
                  setActiveVisualizationId={setActiveVisualizationId}
                />
              )}
