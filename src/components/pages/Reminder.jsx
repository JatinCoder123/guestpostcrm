...
export function ReminderPage() {
  ...
  useEffect(() => {
    ...
    if (urlFilter) {
      ...
      let matchingOption = enhancedDropdownOptions.find(
        (option) =>
      ...
      if (!matchingOption) {
        ...
        matchingOption = enhancedDropdownOptions.find(
          (option) =>
  ...
  async function sendReminder(reminderId) {
    ...
    axios
      .get(`${crmEndpoint}&type=send_reminder&reminder_id=${reminderId}`)
      .then((res) => {
        if (res.data?.success) {
  ...
  useEffect(() => {
    ...
    if (selectedCategory === "pending") {
      ...
      filteredReminders = orderRem.filter((reminder) => {
        ...
        // Check for matches
        const matchesType =
          normalizedReminderType.includes(normalizedSelectedCategory) ||
          normalizedSelectedCategory.includes(normalizedReminderType);

        const matchesLabel =
          normalizedReminderTypeLabel.includes(normalizedSelectedCategory) ||
  ...
  const getDisplayLabel = (type) => {
    ...
    const option = enhancedDropdownOptions.find(
      (option) =>
        option.value === type ||
  ...
  // Calculate stats
  const pendingCount = orderRem.filter(
    (order) => order.status.toLowerCase() === "pending",
  ).length;

  const sentCount = orderRem.filter(
    (order) => order.status.toLowerCase() === "sent",
  ).length;

  const cancelledCount = orderRem.filter(
    ...
    <>
      ...
      {id && open && (
        <ViewReminder
          ...
          loading={loading}
          reminder={reminders.find((reminder) => reminder.thread_id === id)}
        />
      )}
