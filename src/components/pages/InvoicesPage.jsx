...
export function InvoicesPage() {
  ...
  const filteredinvoices = invoices
    ...
    .filter((item) => {
      ...
      if (filters.paymentMethod && filters.paymentMethod !== "all") {
        ...
        if (
          ["paypal", "payoneer", "wise", "indian_upi", "swift_india"].includes(
            filterMethod,
          )
        ) {
    ...
    .filter((item) => {
      ...
      if (filters.hasAttachment) {
        return (
          item.has_attachment || item.attachments || item.attachment_count > 0
        );
      }
      return true;
    });
  ...
  const topPaymentMethod = Object.entries(paymentMethodsStats).sort(
    (a, b) => b[1].amount - a[1].amount,
  )[0] || ["None", { count: 0, amount: 0 }];
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <a
              href="https://www.guestpostcrm.com/blog/one-click-paypal-invoice-creation/"
              target="_blank"
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <span
                          className={`px-3 py-1.5 rounded-full text-sm border flex items-center ${getPaymentMethodColor(invoice.payment_method)}`}
                        >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-sm ${getStatusColor(invoice.status_c)}`}
                      >
                        {invoice.status_c}
                      </span>
                    </td>
        ...
        {filteredinvoices.length > 0 && (
          <Pagination
            slice={"invoices"}
            fn={(p) => dispatch(getInvoices({ page: p }))}
          />
        )}
        {filteredinvoices.length === 0 && (
