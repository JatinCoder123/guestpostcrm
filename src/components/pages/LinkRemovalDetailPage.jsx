...
import {
  useLinkRemoval
} from "../../queries/linkRemoval.queries";
...
const Link = ({ href, children, title, className = "" }) => (
  <a
    ...
    title={title}
    className={`inline-flex max-w-full items-center gap-1 text-blue-600 hover:underline ${className}`}
  >
...
export default function LinkRemovalDetailPage() {
  ...
  /**
   * --------------------------------------------------
   * GET LINK REMOVAL RECORD BY ID
   * --------------------------------------------------
   *
   * This record contains the anchor_text_c and
   * backlink_url_c that we use to determine whether
   * the link still exists on the source page.
   */
  const {
    data: linkRemovalData,
    isPending: isLinkRemovalLoading,
  } = useLinkRemoval(id);

  const linkRemovalRecord =
    linkRemovalData?.records?.[0];


  const isDefaulter =
    backlink?.defaulter === true ||
    String(backlink?.defaulter) === "1";
  ...
  /**
   * --------------------------------------------------
   * NORMALIZE SELECTED LINK DATA
   * --------------------------------------------------
   *
   * Prefer the link-removal record.
   * Fallback to backlink record if necessary.
   */
  const selectedAnchor = String(
    linkRemovalRecord?.anchor_text_c ||
    backlink?.anchor_text_c ||
    ""
  )
    .trim()
    .toLowerCase();
  ...
  const selectedTargetUrl = String(
    linkRemovalRecord?.backlink_url_c ||
    backlink?.backlink_url_c ||
    ""
  )
    .trim()
    .toLowerCase();
  ...
  /**
   * --------------------------------------------------
   * CHECK WHETHER THE EXACT BACKLINK STILL EXISTS
   * --------------------------------------------------
   *
   * We compare BOTH:
   *
   ...
   * 2. target_url
   *
   * This prevents another backlink with the same
   * anchor text from being treated as the same link.
   */
  const backlinkStillExists = links.some((link) => {
  ...
  const removeBacklink = ({
    ...
    if (!backlink || isUpdating) {
      return;
    }

    /**
     * Don't allow removal again if the backlink
     * has already disappeared from the source page.
     */
    if (isAlreadyRemoved) {
      toast.error(
        "This link is already removed."
      );
          ...
          if (isDefaulter) {
            ...
            const [
              ...
              checkOrderAndInvoice({
                checkOrder:
                  Boolean(orderPreference),

                checkInvoice:
                  Boolean(invoicePreference),
              }),
            ...
            setLatestLinkCount(
              refreshedExtraction?.data
                ?.total_links ??
              refreshedLinks.length
            );
          } else {
            navigate(-1);
          }
      ...
      <div className="space-y-5 p-4 md:p-6">
        ...
        <section className="rounded-xl border bg-white shadow-sm">
          ...
          {!isExtractionLoading &&
            !extractionError &&
            isAlreadyRemoved && (
              <div className="m-5 rounded-xl border border-amber-200 bg-amber-50 p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-amber-700">
                      The backlink with anchor text{" "}
                      <span className="font-semibold">
                        "{linkRemovalRecord?.anchor_text_c ||
                          backlink?.anchor_text_c}"
                      </span>{" "}
                      is no longer present on the source page.
                    </p>

                    {selectedTargetUrl && (
                      <p className="mt-2 break-all text-xs text-amber-600">
                        {linkRemovalRecord?.backlink_url_c ||
                          backlink?.backlink_url_c}
                      </p>
                    )}
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-5 py-4">
                              ...
                              >
                                {link.target_url?.length >
                                  35
                                  ? `${link.target_url.substring(
                                    0,
                                    35
                                  )}..`
                                  : link.target_url}
                            ...
                            <td className="px-5 py-4">
                              <Link
                                ...
                                className="text-blue-400 hover:text-blue-300 hover:underline"
                              >
                                {link.source_url?.length >
                                  35
                                  ? `${link.source_url.substring(
                                    0,
                                    35
                                  )}..`
                                  : link.source_url}
              ...
              <div className="bg-slate-50 px-7 py-6">
                ...
                {!preferencesConfirmed ? (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                      <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                        Latest backlink count
                      </p>

                      <p className="mt-2 text-2xl font-bold text-slate-900">
                        {latestLinkCount === null
                          ? "Checking..."
                          : latestLinkCount}
                      </p>
                    </div>
                ...
                {!preferencesConfirmed && (
                  <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Checked options are saved as 1;
                      unchecked options are saved as 0.
                    </p>
                  ...
                  <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mb-5 flex items-center justify-between border-b border-slate-100 pb-4">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          Status is fetched directly from
                          CRM records.
                        </p>
                      </div>
