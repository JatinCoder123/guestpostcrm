...
const ReportPage = () => {
  ...
  const endpointsConfig = {
    email: [
      ...
      { key: "ledger", name: "Email Ledger", icon: "📋", category: "communication", requiresEmail: true },
      { key: "unreplied", name: "Unreplied Emails", icon: "📨", category: "communication", requiresEmail: true },
      { key: "unanswered", name: "Unanswered Emails", icon: "❓", category: "communication", requiresEmail: true },
      { key: "view_email", name: "View Email", icon: "👁️", category: "communication", requiresEmail: true },
      { key: "view_thread", name: "Thread Messages", icon: "💬", category: "communication", requiresEmail: true },
      
      // Email Automation Endpoints
      { key: "ai_reply", name: "AI Reply", icon: "🤖", category: "automation", requiresEmail: false },
      { key: "pre_filled_ai_reply", name: "Pre-filled AI Reply", icon: "💬", category: "automation", requiresEmail: true },
      { key: "thread_reply", name: "Send Reply", icon: "↩️", category: "automation", requiresEmail: false },
      
      // Email Monitoring Endpoints
      { key: "spam_detection", name: "Spam Detection", icon: "🚫", category: "monitoring", requiresEmail: false },
      
      // Email Reminders
      { key: "order_reminder", name: "Order Reminders", icon: "⏰", category: "reminders", requiresEmail: true },
      { key: "deal_reminder", name: "Deal Reminders", icon: "⏰", category: "reminders", requiresEmail: true },
      { key: "payment_reminder", name: "Payment Reminders", icon: "💰", category: "reminders", requiresEmail: true },
      { key: "link_removal", name: "Link Removal", icon: "🔗", category: "maintenance", requiresEmail: true }
    ],
    dashboard: [
      // Core Analytics Endpoints
      { key: "get_contact", name: "Contact Summary", icon: "👤", category: "analytics", requiresEmail: false },
      
      // Sales & Transactions
      { key: "get_orders", name: "Orders", icon: "🛒", category: "sales", requiresEmail: false },
      { key: "get_deals", name: "Deals", icon: "🤝", category: "sales", requiresEmail: false },
      { key: "get_offers", name: "Offers", icon: "🎁", category: "sales", requiresEmail: false },
      { key: "get_invoices", name: "Invoices", icon: "🧾", category: "sales", requiresEmail: false },
      
      // Financial
      { key: "get_credits", name: "Credits", icon: "💳", category: "financial", requiresEmail: false },
      
      // CRM Data
      { key: "get_post_all", name: "CRM Leads", icon: "🏢", category: "crm", requiresEmail: false }
    ]
  };
  ...
  const fetchApiData = async (endpointType, currentFilter) => {
    try {
      ...
      const allEndpoints = [...endpointsConfig.email, ...endpointsConfig.dashboard];
  ...
  const loadAllEndpointStats = async (type) => {
    ...
    try {
      ...
      const successRate = endpoints.length > 0 ? (successfulEndpoints / endpoints.length) * 100 : 0;
      ...
      setAnalyticsData({
        ...
        averagePerEndpoint: endpoints.length > 0 ? Math.round(totalCount / endpoints.length) : 0,
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-1">
                          {endpoint.requiresEmail && (
                            <span className="text-xs bg-gray-100 px-2 py-1 rounded">email</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="text-xs text-gray-500 mt-2">
                        Filter: {stat.apiFilter || getApiFilter(endpoint.key, filter)}
                      </div>
    ...
    return (
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">
          {selectedCard === 'email' ? '📧 Email Analytics' : '📊 Dashboard Analytics'}
        </h3>
        
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-6">
          <div className="text-center p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg border border-blue-200">
            <div className="text-2xl font-bold text-blue-600">{analyticsData.totalCount?.toLocaleString() || 0}</div>
            <div className="text-sm text-blue-700 font-medium">Total Records</div>
          </div>
          <div className="text-center p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
            <div className="text-2xl font-bold text-green-600">{analyticsData.successfulEndpoints || 0}/{analyticsData.totalEndpoints || 0}</div>
            <div className="text-sm text-green-700 font-medium">Active Endpoints</div>
          </div>
          <div className="text-center p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200">
            <div className="text-2xl font-bold text-purple-600">{analyticsData.successRate || '0'}%</div>
            <div className="text-sm text-purple-700 font-medium">Success Rate</div>
          </div>
          <div className="text-center p-4 bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg border border-orange-200">
            <div className="text-2xl font-bold text-orange-600">{analyticsData.growth || '+0%'}</div>
          ...
          <div className="text-center p-4 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg border border-indigo-200">
            <div className="text-2xl font-bold text-indigo-600">{analyticsData.averagePerEndpoint || 0}</div>
            <div className="text-sm text-indigo-700 font-medium">Avg/Endpoint</div>
          </div>
        </div>
    ...
    if (!stat || tableData.length === 0) {
      return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {/* Header with Back Button - Always shown even when no data */}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-100 mt-1">
                    No data available • Filter: {stat?.apiFilter || getApiFilter(currentEndpoint, filter)}
                  </p>
                </div>
              </div>
              <span className={`px-4 py-2 rounded-full text-sm font-medium border backdrop-blur-sm ${
                stat?.success 
                  ? 'bg-green-500/20 text-green-100 border-green-300/50' 
                  : 'bg-red-500/20 text-red-100 border-red-300/50'
              }`}>
                {stat?.success ? 'Active' : 'Failed'}
              </span>
            </div>
          </div>
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-600 mb-4">No Data Available</h3>
            <p className="text-gray-500 text-lg mb-6 max-w-2xl mx-auto">
              {stat?.error 
                ? `There was an error loading data for this endpoint: ${stat.error}`
                : "This endpoint doesn't have any data available for the current filter."
              }
            </p>
            <div className="bg-gray-50 rounded-lg p-6 max-w-2xl mx-auto">
              <h4 className="font-semibold text-gray-800 mb-3">Endpoint Details:</h4>
              <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                <div className="text-left">
                  <span className="font-medium">Endpoint:</span> {currentEndpoint}
                </div>
                <div className="text-left">
                  <span className="font-medium">Category:</span> {stat?.category || 'Unknown'}
                </div>
                <div className="text-left">
                  <span className="font-medium">Filter:</span> {stat?.apiFilter || getApiFilter(currentEndpoint, filter)}
                </div>
                <div className="text-left">
                  <span className="font-medium">Status:</span> 
                  <span className={`ml-2 px-2 py-1 rounded text-xs ${
                    stat?.success ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                  }`}>
                    {stat?.success ? 'Active' : 'Failed'}
                  </span>
                </div>
  ...
  const renderFilters = () => (
    <div className="flex gap-2 p-1 bg-white/20 rounded-lg w-full overflow-x-auto backdrop-blur-sm">
      {filterOptions.map((f) => (
        <button
          key={f.key}
          disabled={loading}
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white">
                {selectedCard === 'email' ? "📧 Email Management" : "📊 Business Dashboard"}
              </h1>
            ...
            <div className="text-right">
              <div className="text-4xl font-bold text-white">
                {loading ? "..." : analyticsData.totalCount?.toLocaleString() || 0}
              </div>
        ...
        {loading && (
          <div className="bg-white rounded-xl p-8 text-center shadow-sm border border-gray-200">
            ...
            <p className="text-gray-600 mt-4">
              Loading {selectedCard === 'email' ? 'email' : 'dashboard'} endpoints...
            </p>
          </div>
        )}
      ...
      <div className="w-full max-w-[95rem] mx-auto">
        {!selectedCard ? (
          <div className="text-center mb-10">
            <h1 className="text-4xl font-bold text-gray-900 mb-3">Business Intelligence Dashboard</h1>
            <p className="text-gray-600 text-lg">Monitor and manage your business operations</p>
          </div>
        ) : (
          <button
